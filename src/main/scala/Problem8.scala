...
@main
def solve_problem8() = {
  val input: Seq[Entry] = Problem8.parse_entries(Problem8.input)
  val singletons = input
    .flatMap(_.output)
    .count(x => List(2, 4, 3, 7).contains(x.segments.size))

  println(singletons)

  val outputs = input.par.map(entry => {
    val mapping = Problem8.valid_mapping(entry.digits)
    val output = entry.output.map(_.translate(mapping).digit)
    output.mkString.toInt
  })
...
case class Entry(digits: Seq[Digit], output: Seq[Digit])

case class Digit(segments: Set[Segment]) {
  def valid: Boolean = Digit.numbers.contains(this)
  def digit: Int = Digit.numbers(this)
  def translate(segment_map: Map[Segment, Segment]): Digit = Digit(
    segments.map(x => segment_map(x))
  )
}
...
object Digit {
  val numbers = Map(
    Digit(
      Set(Segment.A, Segment.B, Segment.C, Segment.E, Segment.F, Segment.G)
    ) -> 0,
    Digit(Set(Segment.C, Segment.F)) -> 1,
    Digit(Set(Segment.A, Segment.C, Segment.D, Segment.E, Segment.G)) -> 2,
    Digit(Set(Segment.A, Segment.C, Segment.D, Segment.F, Segment.G)) -> 3,
    Digit(Set(Segment.B, Segment.C, Segment.D, Segment.F)) -> 4,
    Digit(Set(Segment.A, Segment.B, Segment.D, Segment.F, Segment.G)) -> 5,
    Digit(
      Set(Segment.A, Segment.B, Segment.D, Segment.E, Segment.F, Segment.G)
    ) -> 6,
    Digit(Set(Segment.A, Segment.C, Segment.F)) -> 7,
    Digit(
      Set(
        Segment.A,
        Segment.B,
    ...
    Digit(
      Set(Segment.A, Segment.B, Segment.C, Segment.D, Segment.F, Segment.G)
    ) -> 9
  )
}
...
object Segment {
  def parse(c: Char): Segment = c match {
    case 'a' => A
    case 'b' => B
    case 'c' => C
    case 'd' => D
    ...
    case 'f' => F
    case 'g' => G
    case _   => throw new IllegalArgumentException(s"Invalid segment: $c")
  }
...
object Problem8 {
  ...
  def valid_mapping(digits: Seq[Digit]): Map[Segment, Segment] = (for {
    mapping <- possible_mappings
    if digits.forall(x => x.translate(mapping).valid)
  } yield mapping).head

  def parse_entries(input: String): Seq[Entry] = {
    val lines = input.split("\n").map(_.trim)
    lines.map(parse_entry)
  }

  def parse_entry(line: String): Entry = {
    val splits = line.split('|').map(_.trim)
    val (first_part, second_part) = (splits(0), splits(1))
    Entry(parse_digits(first_part), parse_digits(second_part))
  }

  def parse_digits(digits: String): Seq[Digit] = {
    digits.split(" ").map(parse_digit)
  }

  def parse_digit(digit: String): Digit = {
    Digit((digit.toList.map(Segment.parse)).toSet)
  }
