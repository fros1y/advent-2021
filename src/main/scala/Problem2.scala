...
object Problem2 {
  ...
  case class Position(depth: Int = 0, distance: Int = 0) {
    def move(c: Command): Position = c match {
      case Command.Forward(d) => Position(depth, distance + d)
      case Command.Up(d) => Position(depth - d, distance)
      case Command.Down(d) => Position(depth + d, distance)
    }
  }
